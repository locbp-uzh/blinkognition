...
"""
Label-scrambling control: identical to ML/train.py except that all protein labels
are randomly shuffled BEFORE the train/val/test split, so the model cannot learn
any label-correlated structure.  Expected outcome: AUC ≈ 0.5, chance-level accuracy.
...
def build_model_from_config(model_name, in_channels, num_classes, dropout, model_kwargs, device):
    ...
    return build_from_zoo(model_name, **kwargs).to(device)
...
def main():
    ...
    output_root = io_cfg.get("output_root", cfg.get("output_root", "../../Results/Controls/LabelScrambling"))
    dataset     = data_cfg.get("dataset", cfg.get("dataset"))
    if dataset is None:
        raise ValueError("No dataset specified in config.")
    dataset_keys = list(dataset.keys())
    run_name     = io_cfg.get("run_name", "label_scrambled")
    ...
    print("*** LABEL SCRAMBLING CONTROL — labels are randomly shuffled before splitting ***")
    ...
    traces_path          = data_cfg.get("traces_path")

    print("Loading dataset...")
    X, y, class_map, in_channels, unique_ids = build_dataset_from_keys(
        dataset, traces_path,
        trim_end=trim_end,
        max_traces_per_class=max_traces_per_class,
        random_seed=seed_val,
    )

    # ── THE ONLY MODIFICATION: shuffle all labels before splitting ──
    rng = np.random.default_rng(int(seed_val) if seed_val is not None else 0)
    rng.shuffle(y)
    print(f"Labels shuffled. Class distribution after scrambling: "
          f"{ {class_map[i]: int((y == i).sum()) for i in sorted(class_map)} }")
    # ────────────────────────────────────────────────────────────────

    class_names = [class_map[i] for i in sorted(class_map)]
    print(f"Dataset: {X.shape[0]} samples, {in_channels} channels, {X.shape[2]} timesteps")
    ...
    model_name = model_cfg.get("name", "tcn")
    ...
    if use_augmentation:
        train_loader, val_loader, test_loader, _ytrain = create_dataloaders_with_augmentation(
            ...
            random_seed=seed_val,
            augment_train=True,
            aug_factor=augmentation_cfg.get("aug_factor", 1),
            time_warp_sigma=augmentation_cfg.get("time_warp_sigma", 0.0),
            noise_sigma=augmentation_cfg.get("noise_sigma", 0.0),
            magnitude_jitter=augmentation_cfg.get("magnitude_jitter", 0.0),
            include_mirror=augmentation_cfg.get("include_mirror", True),
            **loader_kwargs,
        )
    else:
    ...
    train_idx, temp_idx = train_test_split(indices, test_size=0.30, stratify=y, random_state=seed_val)
    y_temp = y[temp_idx]
    val_idx, test_idx = train_test_split(temp_idx, test_size=0.50, stratify=y_temp, random_state=seed_val)
    np.savez(os.path.join(run_dir, "split_indices.npz"),
             train_idx=train_idx, val_idx=val_idx, test_idx=test_idx, balance_test=balance_test)
    ...
    model = build_model_from_config(model_name, in_channels, num_classes, dropout, user_kwargs, device)

    lr               = float(opt_cfg.get("lr", 1e-3))
    wd               = float(opt_cfg.get("weight_decay", 0.0))
    label_smoothing  = float(opt_cfg.get("label_smoothing", 0.05))
    compile_enabled  = bool(system_cfg.get("compile", False))
    ...
    print("Starting training on scrambled labels...")
    ...
    if mcd_cfg.get("enabled", True):
        ...
        wasserstein_threshold = mcd_cfg.get("wasserstein_threshold")
    ...
    with open(os.path.join(run_dir, "config_summary.json"), "w") as f:
        json.dump({"run_dir": run_dir, "model": model_name, "scrambled": True,
                   "num_classes": num_classes, "device": str(device)}, f, indent=2)
